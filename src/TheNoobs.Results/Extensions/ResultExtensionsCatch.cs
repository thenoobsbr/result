...
public static partial class ResultExtensions
{
    ...
    public static Result<T> Catch<T, TFail>(
        this Result<T> current,
        Func<TFail, BindResult<T>, Result<T>> fail)
        where TFail : Fail
    {
        if (current.IsSuccess)
        {
        ...
        var bindParameter = current as BindResult<T> ?? (current.IsSuccess ? new BindResult<T>(null, current) : new BindResult<T>(current.Fail));

        if (current.Fail is TFail failResult)
        {
            return fail(failResult, bindParameter);
        }
    ...
    public static async ValueTask<Result<T>> CatchAsync<T, TFail>(
        this Result<T> current,
        Func<TFail, BindResult<T>, ValueTask<Result<T>>> failAsync)
        where TFail : Fail
    {
        if (current.IsSuccess)
        {
        ...
        if (current.Fail is TFail failResult)
        {
            return await failAsync(failResult, bindParameter).ConfigureAwait(false);
        }
